...
export function createServer() {
  ...
  server.tool(
    ...
    async ({ projectId, taskId }) => {
      const result = await tasks.get(projectId, taskId);
      return {
        content: [{ type: 'text', text: JSON.stringify(result, null, 2) }],
      };
    }
  ...
  server.tool(
    ...
    async ({ projectId, title, content, dueDate, priority, tags, reminder }) => {
  ...
  server.tool(
    ...
    async ({ projectId, taskId }) => {
      const result = await tasks.remove(projectId, taskId);
      return {
        content: [{ type: 'text', text: JSON.stringify(result, null, 2) }],
      };
    }
  ...
  server.tool(
    ...
    async ({ keyword, tags, priority }) => {
      const result = await tasks.search(keyword || '', { tags, priority });
      return {
        content: [{ type: 'text', text: JSON.stringify(result, null, 2) }],
      };
    }
