...
import { apiRequest, parseReminder, parsePriority, formatPriority, shortId, isShortId } from './core.mjs';
...
export async function create(projectId, title, options = {}) {
  const resolvedProjectId = await resolveProjectId(projectId);
  const input = { title, projectId: resolvedProjectId };
 ...
 * @param {string} projectId - Project ID or short ID (empty string for inbox)
 * @returns {Promise<string>} - Full project ID
 */
async function resolveProjectId(projectId) {
  // Empty string or missing means inbox
  if (!projectId) {
    return '';
  }
