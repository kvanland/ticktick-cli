...
// Config paths (XDG-compliant with legacy fallback)
const XDG_CONFIG_DIR = join(process.env.XDG_CONFIG_HOME || join(homedir(), '.config'), 'ticktick');
const LEGACY_CONFIG_DIR = join(homedir(), '.clawdbot', 'ticktick');

// Determine which config directory to use
function getConfigDir() {
  // Prefer legacy if it exists (migration support)
